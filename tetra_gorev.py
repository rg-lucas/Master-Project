...
class robot_camera():
    ...
    def camera_cb(self, mesaj):
        ...
        if len(cnts) > 0:
            ...
            cv2.putText(self.cap, "X : " + str(round(x, 2)), (int(x) + int(radius) + 5, int(y)),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
            cv2.putText(self.cap, "Y : " + str(round(y, 2)), (int(x) + int(radius) + 5, int(y) + 35),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
            ...
            if radius > 150:
                ...
                self.hiz_mesaji.linear.x = 0.50  # ileri gitmesi icin
                self.hiz_mesaji.angular.z = 0.0 #-hata_x / 100
                self.pub.publish(self.hiz_mesaji)
